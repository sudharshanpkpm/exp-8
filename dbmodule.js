var databaseUrl = "mongodb://127.0.0.1:27017/mydb";
var mongojs = require("./node_modules/mongojs");
var db = mongojs(databaseUrl);
var collections = db.collection("user");
console.log("Connected");

exports.authenticateUser = function(username, email, password, response) 
{
    console.log(username);
    console.log(email);
    console.log(password)
    db.user.find({ "username": username, "email": email, "password": password },
        function(err, op) {
            
            if (err || !op) {
                response.write("..Not authorized user" || err);
                response.end();
            } else if (op.length == 0) {
                response.write("Not authorized user");
                response.end();
            } else {
                response.write("Authorized user");
                response.end();
            }

        });
}
exports.saveUser = function(username, email, password, response) {
console.log('Saving user to mongo');
db.user.insert({ "username": username, "email": email, "password": password },
function(err, saved) 
{
    if (err || !saved)
        console.log(err);
    else
        response.write("User Saved");
        response.end();
         });
}
